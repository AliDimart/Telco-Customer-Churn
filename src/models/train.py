...
def train_model(X_train: pd.DataFrame, y_train: pd.Series, params : dict = None) -> Pipeline:
    ...
    Args:
        ...
        params (dict): Model Hyperparametrs
    """
...
def build_model(X_train: pd.DataFrame, y_train: pd.Series, params: dict = None) -> Pipeline:
    '''
    Returns an XGBoost model without training on data. 
    Args:
        X_train (pd.DataFrame): Feature dataset
        y_train (pd.Series): Target column
        params (dict): Model Hyperparametrs
